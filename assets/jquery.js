...
$("#change-jquery").on("click", () => {
  $(jqueryId).slideDown();
  $(vanillaId).slideUp();
  active = "jquery";
});

$("#change-vanilla").on("click", () => {
  $(vanillaId).slideDown();
  $(jqueryId).slideUp();
  active = "vanilla";
});
...
$("#function-1Q button").click(() => {
  ...
  $("#function-4Q button").click(() => {
    $("#function-4Q .modal__example__try").prepend(
      "<p>Hello World prepended!</p>"
    );
  });
  ...
  $("#function-7Q button").click(() => {
    ...
    $("#function-7Q .modal__example__try p")
      .first()
      .clone()
  ...
  $("#event-5Q button").click(() => {
    $(document).mousemove(function mouseQuery() {
      $("#event-5Q span").text(
        "Mouse moving: " + event.pageX + ", " + event.pageY
      );
  ...
  $("#event-10Q select").change(() => {
    $("p", $(event.target).parent()).text(
      "Changed to: " + $("#event-10Q select").val()
    );
  });
  ...
  $("#event-13Q ol").click(() => {
    $(event.target)
      .parent()
      .next()
      .text("Item clicked: " + $(event.target).text());
  });
