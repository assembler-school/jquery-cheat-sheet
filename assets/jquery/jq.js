...
function loadAction() {
	...
	$(".result").append(
		'<img src="assets/ShinChan1.jpg" alt="chin chan" class="imag">'
	);
...
function isChange() {
	$(".result").text("");
	$(".result").append(
		"<select class='sOptions'><option class='Options'> option 1</option><option class='Options'> option 2</option></select>"
	);
...
function removingClass() {
	...
	$(".result").text(
		"<p class='result newClass'>to many classse ,plese remove one</p>"
	);
...
function newAtribute() {
	...
	if ($(".run:disabled")) {
		...
		$(".result").append(
			"<button class='run' id='newbut'>Make the button able here!!!,come on!!</button>"
		);
...
function dataSrc() {
	...
	$(".result").append("<p>");
	$(".result p").text(
		'<img src="assets/ShinChan1.jpg" alt="chin chan" class="imag" data-src="I,m data">'
	);
...
function runArray() {
	...
	$(".result").append(
		"<ul><li>hola</li> <li>ketal?</li> <li>estaís?</li> </ul><button id='lis' class='run'>styles</button>"
	);
...
function sonNode() {
	...
	$(".run").on("click", function () {
		...
		$(".run").text("The console.log show: ");
		$(".result").text(
			"div.t1, div.vanillaBox, div.jQueryBox, button.ejempleButJq, button.run, button.ejempleButVani, p.result"
		);
		$(".result").css("color", "violet");
...
function classNone() {
	...
	$(".result").append(
		'<button class="run" id="but">show the title and change styles</button>'
	);
...
function selecD() {
	...
	$(".result").append(
		" <select class='sOptions'><option class='Options'>goode morning</option><option class='Options'>good night</option><option class='Options'>I,m finally crazy?</option><option class='Options'> How long?</option></select>"
	);
	$(".result").prepend(
		'<button class="run" id="but">Witch I selected ? </button>'
	);
	$("#but").on("click", function () {
