...
function createGetParentElement(r) {
  ...
  $(par).text(`This one is pretty simple on Js: let x = btn1.parentElement;  `);
...
function createCollectionOfKids(r) {
  ...
  let par = document.createElement("p");
  $(par).text(
    `In order to select them you just need to do as follows, we will delete them to show the selection worked: par.childNodes  `
  );
  $(par).css("display", "none");
  $(topSlider).append(par);
...
function createAllElementsClass(r) {
  ...
  let par = document.createElement("p");
  $(par).text(
    `This can be done with classes and tags. As I´m lazy I´m gonna use all P elements instead of creating elements with an specific class. There are many ways of doing this but my favourite is: document.querySelectorAll("p"), this however returns a node list, so as usual to interact with the elements is necessary to convert them to an array.`
  );
  $(par).css("display", "none");
  $(topSlider).append(par);
...
function createIdElements(r) {
  ...
  let par = document.createElement("p");
  $(par).text(
    `This one of the most intuitive ways of selecting a single element: document.getElementById("id")`
  );
  $(par).css("display", "none");
  $(topSlider).append(par);
...
function getAllThemElements(r) {
  ...
  let j = [
    "Ich bin Lord Voldemord",
    "Ich will deutsch sprechen",
    "Why are those guys speaking german?",
  ];
  j.forEach((element) => {
...
function createOptionSelect(r) {
  ...
  $(par).append(
    "<select name=rockets id=rockets><option value=SpaceX>SpaceX</option> <option value=blueOrigins>blue Origins</option></select>"
  );
  ...
  $(parBo).append(
    "<select name=rockets id=fruit><option value=Apple>Apple</option> <option value=Banana>Banana</option></select>"
  );
...
function changeHref(r) {
  ...
  let par = document.createElement("p");
  $(par).text(
    `This how you change the href from the first anchor element: Array.from(document.querySelectorAll("a"))[0].href = "https://www.youtube.com/watch?v=dQw4w9WgXcQ&ab_channel=RickAstley"`
  );
  $(par).css("display", "none");
  $(topSlider).append(par);
  ...
  let btn1 = document.createElement("button");
  $(btn1).text(
    "Click the first attribute after pressing this button to see the change"
  );
  $(btn1).addClass("btn1");
  $(par).append(btn1);
  ...
  $(x).attr(
    "href",
    "https://www.youtube.com/watch?v=pM3JPhWApvY&ab_channel=wiceone"
  );
  $(x).text("Extremoduro");
  $(par).append(x);
...
function showMeThemInputs(r) {
  ...
  let par = document.createElement("p");
  $(par).text(
    `This one is fairly simple in js: document.querySelector("input").value`
  );
  $(par).css("display", "none");
  $(topSlider).append(par);

  $(par).append(
    "<form id=l> <label for=textMe>Enter Input Here</label><input type=text id=textMe name=textMe><button id=testComm>I´ll show you what I mean</button></form>"
  );
  ...
  $(parBo).append(
    "<form id=u> <label for=textIt>Enter Input Here</label><input type=text id=textIt name=textIt><button id=testCom>I´ll show you what I mean</button></form>"
  );
...
function createAnimationFun(r) {
  ...
  let par = document.createElement("p");
  $(par).text(
    `It was ask to be animated 2 seconds after page load. For demo purposes we are gonna do it 2 seconds after button click.In order to do that we are gonna put the animation inside a set timeout.`
  );
  $(par).css("display", "none");
  $(topSlider).append(par);
  ...
  $(parBo).append(btn2);
  $(parBo).append(
    "<p id=carmen style=background:white>My name is Carmen San Diego<p>"
  );
