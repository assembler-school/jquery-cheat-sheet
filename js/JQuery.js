...
function queryButtonCreate(btn2, parBo) {
  $(parBo).append(
    "<button class=clicky>Click me to run the event listener.</button>"
  );
  $(".clicky").on({
    click: function () {
...
function formCreations(parBo, btn2) {
  $(btn2).append(
    "<form class=elemental id=elemental><label for=fname>Random form</label> <input type=text id=fname  name=fname> <button class=op>Execute</button></form>"
  );
  $(".elemental").on({
    submit: function () {
...
function selectChanges(btn2, parBo) {
  $(parBo).append(
    "<label for=car>Choose a car</label><select name=car id=car class=car><option value=Tesla>Tesla</option><option value=Ferrari>Ferrari</option>"
  );
  $(".car").on({
    change: function () {
      let x = document.createElement("p");
...
function lastChecks(btn2, parBo) {
  $(parBo).append(
    "<input type=checkbox  class=car2 id=car2 name=car2 value=Car> <button class=checkings2>Check</button><label for=car1>Just check me</label>"
  );
  $(".checkings2").on({
    click: function () {
      $(".car2").on({
...
function itIsFinallyOver(btn2, parBo) {
  $(parBo).append(
    "<ul class=done><li>Element1</li><li>Element2</li><li>Element3</li></ul>"
  );
  $("li:nth-child(2)").on({
    click: function () {
      $(this).css("color", "blue");
